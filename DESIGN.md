...
> MobaXterm 的 Rust 跨平台复刻，支持 Linux / macOS / Windows

---

## 1. 项目定位

NewMob 是一个面向运维和开发人员的跨平台桌面应用，提供：
...
```css
/* 核心颜色变量 — 实现时必须遵循 */
--moba-chrome-bg: #f0f0f0;          /* 窗口背景 */
--moba-accent: #2b5d8b;             /* 主色调（深蓝） */
--moba-accent-soft: #5586b5;        /* 辅助色 */
--moba-tab-active: #ffffff;          /* 活跃标签背景 */
--moba-tab-inactive: #d6e0ec;        /* 非活跃标签 */
--moba-sidebar-bg: #f5f6f8;         /* 侧边栏背景 */
--moba-term-bg: #1d1f21;            /* 终端背景（深色） */
--moba-term-text: #eaeaea;          /* 终端文字 */
--moba-term-prompt: #62d36f;        /* 终端 prompt 用户名（绿色） */
--moba-term-path: #83a7d8;          /* 终端 prompt 路径（蓝色） */
--moba-hover: #eaf2fb;              /* 悬停高亮 */
--moba-selected: #cfe2f7;           /* 选中高亮 */

/* 字体 */
font-family: 'Segoe UI', 'Inter', 'Helvetica Neue', sans-serif;  /* UI 字体 */
.moba-mono: 'Consolas', 'Menlo', 'DejaVu Sans Mono', monospace;  /* 等宽字体 */
```

**给 AI Agent 的原型使用指南：**

1. 实现任何前端组件前，先读取对应的原型文件了解布局结构和交互模式
2. CSS 变量和组件类名（`moba-btn`、`moba-tab`、`moba-tree-row` 等）应迁移到 TailwindCSS + CSS 变量方案
...
### 上下文
- 项目：NewMob (Tauri 2 + React + TypeScript)
